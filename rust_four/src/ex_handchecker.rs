...
use crate::ex_hand::{Hand, HANDMAXSCORE, HANDVARIANT};
use crate::ex_set::{Meld, Pair, Set, SETNUM};
use crate::tile::{Tile, TileColor};

#[derive(Clone, Hash, PartialEq, Eq, Debug)]
pub struct HandList(BitArr!(for HANDVARIANT, in u64));
...
impl std::ops::Deref for HandList {
    type Target = BitArr!(for HANDVARIANT, in u64);
...
impl HandChecker {
    ...
    pub fn hands(&self) -> HandList {
        ...
        self.all_revealed_concealed(&mut checker);
        self.kongs(&mut checker);
        ...
        match (
            ...
            checker[Hand::FourKongs as usize],
        ) {
...
impl HandChecker {
    ...
    fn pure_mix(&self, checker: &mut HandList) {
        ...
        if let true = self
            .melds
            .iter()
            .all(|m| m.head().is_same_color(*self.pair))
        {
    ...
    // 全求人、不求人
    fn all_revealed_concealed(&self, checker: &mut HandList) {
        match self.pair.is_concealed() {
            true => match self.melds.iter().all(|m| m.kind().is_concealed()) {
                true => checker.set(Hand::AllConcealed as usize, true),
                false => (),
            },
            false => match self.melds.iter().all(|m| m.kind().is_revealed()) {
                true => checker.set(Hand::AllConcealed as usize, true),
                false => (),
            },
        }
    ...
    // 槓子類
    fn kongs(&self, checker: &mut HandList) {
        let kongs = self.melds.iter().filter(|m| m.kind().is_kong()).count();
        match kongs {
            4 => checker.set(Hand::FourKongs as usize, true),
            3 => checker.set(Hand::ThreeKongs as usize, true),
            2 => checker.set(Hand::TwoKongs as usize, true),
            1 => checker.set(Hand::OneKong as usize, true),
            _ => (),
        }
    }
}
