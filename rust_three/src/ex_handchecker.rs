...
impl HandChecker {
    ...
    pub fn hands(&self) -> HandList {
        ...
        match (
            ...
            checker[Hand::ThreeKongs as usize],
        ) {
            (false, false) => (),
            _ => {
                checker
    ...
    fn shift_pungs(&self, checker: &mut HandList) {
        ...
        let is_shifted_pungs = |m1: Meld, m2: Meld| -> bool {
            match (m1.kind().is_pung(), m2.kind().is_pung()) {
                (true, true) => {
                    match (
                        m1.head().is_ascending(m2.head()),
                        m2.head().is_ascending(m1.head()),
                    ) {
                        (false, false) => false,
                        _ => true,
                    }
    ...
    fn all_revealed_concealed(&self, checker: &mut HandList) {
        match self.pair.is_concealed() {
            ...
            false => match self.melds.iter().all(|m| m.kind().is_revealed()) {
                true => checker.set(Hand::AllRevealed as usize, true),
                false => (),
            },
        }
    }
